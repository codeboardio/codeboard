  ...
  db.Project
    .findOne(
    {
      ...
      include: [
        ...
        {
          model: db.Project.scope('course'), as: 'Parent', required: false,
          attributes: ['id']
        }
      ]
    })
  ...
  db.Project
    .findOne(
    {
      ...
      include: [
        ...
        // todo falls wir Konfiguration künftig ersetzen, hier Foreign-Table (bsp. `options`) laden (Janick, 16.01.20)
...
var createProjectForUser = function (req, res) {
  ...
  // create new project (course, task, whatever)
...
var _runActionOnProject = function (req, res) {
  ...
  if (action === 'compile' && action === 'compileAndRun') {
    ...
    // read configuration
    let config = extractConfigurations(data);


    // es braucht zwei Arten von Aufrufen: Tipps laden / Anfrage senden
...
let updateHelpRequest = function(req, res) {
  ...
  db.HelpRequest.update(
      { status: req.body.status },
      { where: {id: req.body.helpRequestId } }
  )
    .then(function(request) {

      db.HelpRequest.findByPk(req.params.projectId)
 ...
 *
 * when dealing with courses we always have to use .scope("course")
 *
 * todo maybe create a separate controller for courses
 *  but sometime the separate project-type uses the same code
 */
...
let getUserCourses = function(req, res) {
    ...
    db.User.findOne({
      ...
      include: [
        {
          model: db.Project.scope('course'), as: 'ownerSet', required: false,
          include: [
            // {model: db.User, as: 'ownerSet', attributes: ['username']}
            {model: db.User, as: 'userSet', attributes: ['username']} // we no longer show the list of users; it makes the query too slow
          ],
        ...
        {
          model: db.Project.scope('course'), as: 'userSet', required: false,
          attributes: ['id', 'projectname', 'language', 'description', 'createdAt', 'isPrivate']
        }
      ]
    })
...
let getCourse = function(req, res) {
  ...
  db.Project.scope("course").findOne( {
      where: {id: courseId},
      attributes: ['projectname', 'language', 'description', 'isSubmissionAllowed']
    })
