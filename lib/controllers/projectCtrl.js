...
var db = require('../models'),
  ...
  requestHelpSrv = require('../services/requestHelpSrv');
  ...
  templateSrv.createProjectFromTemplate(
    ...
    .then(function (idOfNewProject) {
      if (idOfNewProject !== -1) {
        ...
        res.status(201).json({message: 'Successfully created project.', id: idOfNewProject});
      }
      else {
        ...
        .then(function(result) {
           console.log(result);
            ...
            var _submissionRecord = {
                ...
                // hiddenFilesDump: data.userRole === 'owner' ? JSON.stringify([]) : JSON.stringify(_hiddenFileSet) brauchen wir wahrscheinlich nicht mehr
            };
            ...
            if (submissionRecord.hasResult && submissionRecord.ltiSessionId !== -1) {
                ...
                ltiSrv.submitGradeToLtiToolConsumer(projectId, _ltiId, _ltiUserId, _ltiNonce, submissionRecord.testResult)
                    .then(function (result) {
                        ...
                        res.status(status).json({msg: result.msg});
                    });
            }
            else {
 ...
 * @private
 */
var _createSubmissionForInputOutputMatching = function (req, res) {
  var projectId = req.params.projectId;
  ...
  projectSrv
    ...
    .then(function (hiddenFileSet) {
       ...
       * Notes:
       *
       * -  Inputs müssen in der richtigen Reihenfolge angegeben werden. Sobald ein Test zutrifft wird der Test
       *    abgebrochen und die entsprechende Meldung ausgegeben. Daher sollte jeweils an erster Stelle die Überprüfung
       *    der korrekten Lösung stattfinden und an letzter Stelle eine Überprüfung, welche sätliche übrigen Resulate
       *    abfängt.
       *
       * -  Inputs und Outputs können sowohl als Array wie auch als Strings angegeben werden.
       *    Arrays können Problemlos in Strings umgewandelt werden
       *
       *    LineByLine-Check ist vorerst nicht notwendig, unterstützt aber die Idee, künftig Outputs auf Bassis von
       *    Zeilen zu überprüfen. Bin mir jedoch noch nicht sicher wie das genau aussehen soll.
       *
       * -  Was passiert wenn kein Test zutrifft? Ist die Aufgabe dann ebenfalls korrekt?
       *    Bsp. Als Lösungsansatz wird nicht eine richtige Lösung gesucht, sondern überprüft ob falsche Werte nicht auftretten!
       *      ->  Ist auch so möglich ... als letzte Bedigung regex mit allen möglichen Werten. Gegenteil vom normalen Vorgehen (siehe 1)
       */
        ...
        'ioTests': [
          {
            'inputs': ['5', '4'],
            'outputs': [
              { 'output': '9', 'pass': true, 'note': 'Super', 'type': 'strict' },
              { 'output': '1', 'pass': false, 'note': 'Das ist falsch', 'type': 'flexible' },
              { 'output': '.*', 'pass': false, 'note': 'Unbekannter Fehler', 'type': 'regex' }
            ]
          },
          {
            'inputs': ['12', '34'],
            'outputs': [
              { 'output': '9', 'pass': true, 'note': 'Genau', 'type': 'strict' },
              { 'output': '(46)', 'pass': false, 'note': 'Dies ist falsch', 'type': 'regex' },
              { 'output': '.*', 'pass': false, 'note': 'Wo liegt blos der Fehler?', 'type': 'regex' }
            ]
          }
        ],
        'onSuccessNote': 'Gratulation, du hast die Aufgabe mit Bravour gemeistert',
        'onFailureNote': 'Deine Aufgaben enthält noch Fehler. Bitte behebe diese, damit du die Aufgabe abschliessen kannst'
      };
      ...
      return io.ioTests.reduce((promiseChain, ioTest) => {

        // set input for this run
        data.input = (ioTest.inputs.length > 0) ? ioTest.inputs.join('\n') + '\n' : '\n';
        ...
        return promiseChain.then(function() {
          ...
          return mantraSrv.compile(data)
              .then(function(executionResult) {
                ...
                // escape string from all three types of line breaks
                ...
                for (let i = 0; i < ioTest.outputs.length; i++) {
                  ...
                  // test depends on the type (strict, flexible, regex)
                  switch (testCase.type) {
                      ...
                      case "flexible":
                          ...
                          testCase.output = testCase.output.replace(/[\\$'"]/g, "\\$&").replace(/\s/g, '');
                      ...
                      case "regex":
                          ...
                          let isRegexValid = true, regex;

                          // check if regex is valid
                          try {
                              regex = new RegExp(testCase.output);
                          } catch (e) {
                              isRegexValid = false;
                          }

                          if(isRegexValid && regex.test(output)) {
                              return testCase;
                          }
                          break;
                  }
                ...
                // todo was passiert, wenn kein Test zutrifft.. ?
                return Promise.reject('Fehler bei der Überprüfung. Bitte wende dich an den Kursleiter.. Kein Output trifft zu');
...
let createHelpRequest = function(req, res) {
  ...
  let _requestHelpRecord = {
    status: 'unanswered',
    userNote: "Ich weiss nicht wie weiter",
    teacherNote: "Wie kann ich helfen?!",
    userFilesDump: JSON.stringify(data.filesInDefaultFormat),
    // hiddenFilesDump: data.userRole === 'owner' ? JSON.stringify([]) : JSON.stringify(_hiddenFileSet) brauchen wir wahrscheinlich nicht mehr
    projectId: projectId,
  ...
  db.HelpRequest.create(_requestHelpRecord)
...
let getAllHelpRequests = function(req, res) {
  ...
  requestHelpSrv.getAllHelpRequestsForProject(projectId, _viewQuery, _userIdQuery)
      .then(function(data) {
        res.status(200).json(data);
      })
      .catch(function(err) {
...
let getHelpRequest = function (req, res) {
  ...
  requestHelpSrv.getHelpRequestById(helpRequestId)
