...
var createSubmission = function (req, res) {
  ...
  projectSrv.getAllHiddenFilesOfProject(projectId, false)
      .then(function (hiddenFileSet) {
        ...
        return submissionSrv.getUserSuccessfulSubmissions(projectId, userId, courseId)
      })
      .then(function (successfulSubmissions) {

        let submitToLti = successfulSubmissions && successfulSubmissions.length > 0;
        ...
        return ioTestingSrv.testProjectForSubmission(data)
            .then(function (testResult) {
              ...
              _submissionRecord.numTestsPassed = ioTests.filter(test => test.status === 'success').length;
              _submissionRecord.numTestsFailed = ioTests.filter(test => test.status !== 'success').length;
              _submissionRecord.testResult = ioTests.length ? (1 / ioTests.length) * _submissionRecord.numTestsPassed : 0;
