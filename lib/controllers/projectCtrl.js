...
var _runActionOnProject = function (req, res) {
  ...
  if (action === 'compile') {
    ...
    // read configFile to get testMethod
    let configFile = data.files.find(file => file.filename === 'Root/codeboard.json'),
        configObject = {};

    try {
        configObject = JSON.parse(configFile.content);
    } catch (e) {
        console.log("Fehler in der Config-Datei"); // todo was machen wir in diesem Fall?
    }

    // NOTE: testMethod kann sein: ioTests, unitTests, manuelTests
    data.testMethod = configObject.testMethod;

    if(configObject.testMethod === 'ioTests') {

        // add ioTests to the data array
        data.ioTests = configObject.ioTests;

        // data.action = 'ioTest'; // todo zuerst mantra diesen actionTyp beibringen

        ioTestingSrv.test(data)
            .then(function (result) {
              logSrv.addActionLog(req, start, data, '');
              res.send(result);
            }).catch(function(err) {
...
var createSubmission = function (req, res) {
  ...
  if (_dynamicLanguageProjects.indexOf(req.body.language) !== -1) {
    ...
    _createSubmissionForInputOutputMatching(req, res);
  }
