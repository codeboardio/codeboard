...
var getFullProject = function (req, res) {
  ...
  return db.Project.findOne( {
    ...
    include: [
      { model: db.Course, as: 'courseSet', required: false },
...
var getLimitedProject = function (req, res) {
  ...
  var courseId = req.query.courseId || null;

  // if project id doesn't exist, show 404
  return db.Project.findByPk(projectId, {
    attributes: ['id', 'projectname', 'language', 'description', 'lastUId', 'isSubmissionAllowed'],
    include: [
      { model: db.Course, as: 'courseSet', where: {id: courseId}, required: false },
...
var getUserProject = function(req, res) {
  ...
  return db.UserProject.findOne({
      ...
      include: [ { model: db.Course, as: 'course', attributes: ['id', 'coursename' ] } ]
    })
...
var putProjectForUser = function (req, res) {
  ...
  // get the courseId
  var courseId = req.body.course.id || -1;
  ...
  if(req.isAuthenticated()) {
      ...
      return db.LtiSession.create({
        ...
        .then(function(ltiSession) {
          ...
          var _uriParams = encodeURI('contextId=' + _contextId + '&ltiSessionId=' + ltiSession.id + '&ltiUserId=' + _userId + '&ltiNonce=' + req.body.oauth_nonce + '&ltiReturnUrl=' + _returnUrl);
          ...
          if(req.body.context_id) {
            res.redirect('/courses/' + encodeURIComponent(req.body.context_id) + '/projects/' + _projectId + '?' + _uriParams);
          } else {
  ...
  projectSrv.getAllHiddenFilesOfProject(projectId, false)
    .then(function (hiddenFileSet) {
      ...
      let _requestHelpRecord = {
        ...
        courseId: req.body.course.id ? req.body.course.id : null // todo -1 doesnt seems to work here
      };
...
var getUserProjectForProject = function(req, res) {
  ...
  // extract the courseId from the route
  var _courseId = req.params.courseId || -1;

  // variable to store the userProject retrieved from the DB
  var _userProject;

  db.UserProject.findOne({
      where: {id: _userprojectId, courseId: _courseId},
