...
const errAndExcHelpSrv = require('../services/errAndExcHelpSrv');
...
var createSubmission = function (req, res) {
  ...
  data.userId = req.user ? req.user.id : -1;
  data.courseId = req.body.course.id ? req.body.course.id : null;
  ...
  projectSrv
    ...
    .then(function (hiddenFileSet) {
      ...
      .then(function(testResult) {
        ...
        // result
        _submissionRecord.hasResult = (testResult.ioTests.length > 0);
        _submissionRecord.numTestsPassed = testResult.ioTests.filter(test => { return (test.status === 'success'); }).length;
        _submissionRecord.numTestsFailed = testResult.ioTests.filter(test => { return (test.status !== 'success'); }).length;
        _submissionRecord.testResult = 1 / testResult.ioTests.length * _submissionRecord.numTestsPassed;
...
let getHelpForCompilation = function(req, res) {
  ...
  errAndExcHelpSrv.getHelpForCompilationErrors(data.compilation.output, data.language, data.courseId, req.user.id)
      .then(function(response) {
        res.status(200).send(response);
      })
      .catch(function(err) {
        res.status(500).json({msg: "Error creating help request"});
      });
};
