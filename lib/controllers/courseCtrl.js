...
let getCourseHelpRequests = function(req, res) {
    ...
    submissionSrv.getAllSubmissionsForCourse(courseId, _viewQuery, _userIdQuery)
        .then(function (submissions) {
            return helpRequestSrv.getAllHelpRequestsForCourse(courseId, _viewQuery, _userIdQuery)
                .then(function (data) {
                    data.forEach(function (helprequest) {
                        var bestSubmission = submissions.filter(submission => submission.user.userId === helprequest.user.userId && submission.project.id === helprequest.project.id).reduce((prev, current) => {
