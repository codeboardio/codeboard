...
let updateLtiUserPassword = function(req, res, next) {
  ...
  //
  userSrv.updateLtiUsersPassword(userId, ltiSrv.generatePasswordFromLtiData(lisPersonSourcedid))
      .then(user => {
        if(user) {
          next();
        } else {
          throw { name: "LtiPasswordChangeFailed", message: "Could not change password" };
        }
      })
      .catch(err => {
        console.log(err);
        return res.status(401).json({
          message: 'LTI user login not working. User: ' + userId + ", " + lisPersonSourcedid,
          authenticated: false
        });
      });
};
