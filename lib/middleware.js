...
var isLtiUserElseNextRoute = function(req, res, next) {
  ...
  if(!(ltiData.hasLtiData)) {
    ...
    ltiSrv.isValidLtiUserProjectSession(projectId, ltiData.ltiSessionId, ltiData.ltiUserId, ltiData.ltiNonce)
      .then(function(isValidLtiUserSession) {

        if(isValidLtiUserSession) {

          if(isValidLtiUserSession.courseId !== req.body.courseId) {
            return Promise.reject("Access rejected because user's supplied LTI session data is not valid for this course.");
          }
