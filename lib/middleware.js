...
var isValidCourseId = function(req, res, next) {
  ...
  var projectId = req.params.courseId;

  console.log(req.params);

  projectSrv.isValidProjectId(projectId, 'course')
    .then(function(isValid) {
      if(isValid)
        next();
      else {
 ...
 *
 * todo vielleicht in lti service verschieben
 *
 * @param req
 * @param res
 * @param next
...
var isLtiUserCreatableElseNextRoute = function(req, res, next) {
  ...
  // create a password from the source did .. todo create function for this purpose
  let password = req.body.lis_person_sourcedid;
  password = password.replace(/[&\/\\#,+()$@\-~%.'":*?<>{}]/g,'');
  password = password.substring(5, 15);

  // todo what happens if the moodle user has already an account with his email?
