...
passport.use('lti-strategy', new LocalStrategy({
    ...
    passwordField: 'lis_person_sourcedid', // this is the virtual field on the model // todo was nehmen wir als password .. oder ohne?
  ...
  function(req, username, password, done) {
    ...
    if(!ltiSrv.isRequestAuthorized(req)) return done(null, false, {message: 'LTI users can only be logged in in the context of an LTI request.'});
    ...
    db.User.scope('ltiUser').findOne({
      ...
      .then(function(user) {
          ...
          // todo haben wir nun ein password oder nicht?
