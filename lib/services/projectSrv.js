...
var setUsersOfProject = function(project, listOfUserNames, asOwners) {
  ...
  return db.User
    .findAll({where: {username: {in: listOfUserNames}}})
    .then(function(users) {
      if(users === null) {
        return project;
      }
      ...
      db.File
        ...
        .then(function(file) {
          return file.updateAttributes({
            filename: updateValues[file.id].filename,
            path: updateValues[file.id].path,
            uniqueId: updateValues[file.id].uniqueId,
            parentUId: updateValues[file.id].parentUId,
            isFolder: updateValues[file.id].isFolder,
            content: updateValues[file.id].content,
            isHidden: updateValues[file.id].isHidden
          });
        })
        .then(function(file) {
