...
let compileTest = function (data) {
    ...
    return mantraSrv.compile(data)
        .then(function (compResult) {
            ...
            // NOTE: da wir momentan nur Java (als nicht-dynamisch Programmiersprache) anbieten können wir das
            // so machen. Ansonsten wäre es besser, dies auf Seite von Mantra (entsprechende Language-Files um
            // Funktion erweitern) zu erledigen.
            if(data.language === 'Java') {
                let output = escapeFromLineBreaks(compResult.output).trim();

                // read and delete number of errors
                let numErrorRegex = new RegExp("(\\d+.errors?)$", "g");
                output.replace(numErrorRegex, "");
                testData.numErrors = output.match(numErrorRegex);

                // regex searches for `./Root/Main.java:12: error:`
                let regex = new RegExp("(\\..+\\.java:\\d+: error:)", "g"),
                    regexResult = output.split(regex).filter(Boolean); // remove empty values with `filter`

                testData.outputArray = [];
                for (let i = 0; i < regexResult.length; i += 2) {
                    testData.outputArray.push(regexResult[i] + regexResult[i + 1]);
                }

                console.log(testData.outputArray);
                console.log(testData.numErrors);
            }
            ...
            if(compResult.compilationError) {
                testData.tests.forEach(function(test) {
                    // if output is not segmented, create array with the output
                    if(typeof testData.outputArray === 'undefined') {
                        testData.outputArray = [testData.output];
                    }
...
let getTests = function(data) {
    ...
    if(data.language === 'Java' && codeboardConfig.checkCompilation !== false) {
    ...
    if(codeboardConfig.ioTests.length > 0) {
        codeboardConfig.ioTests.forEach(function (ioTest) {
            result.tests.push( Object.assign({
                'name': '',
                'method': 'ioTest',
