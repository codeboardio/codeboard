...
const compilationErrorSrv = require('./compilationErrorSrv');
    ...
    return mantraSrv.compile(data)
        .then(function (compResult) {
            ...
            if (compResult.compilationError) {
                ...
                return compilationErrorSrv.onCompilationError(data.projectId, data.userId, data.courseId, filesInDefaultFormat, compResult.output)
                    .then(function(compilationError) {

                        // add compilationErrorId in order make message rating
                        if(compilationError) {
                            testData.compilationErrorId = compilationError.id;
                        }

                        // todo remove the old regex testing
