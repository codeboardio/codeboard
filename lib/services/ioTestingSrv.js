    ...
    return mantraSrv.compile(data)
        .then(function (compResult) {
            ...
            if (compResult.compilationError) {
                ...
                return compilationSrv.onCompilationError(data.projectId, data.userId, data.courseId, filesInDefaultFormat, compResult.output, compilationOutputSeparation(compResult.output))
...
let ioTesting = function (data, testSet, lastCompilationId = 0) {
    ...
    if(languageSrv.isDynamicLanguage(data.language) && lastCompilationId === 0) {
        Promise.reject("Error reading the tests");
    }

    // do io-test asynchronously one after another
    let i = 0;
    let appropriateTests = [];
    return testSet.reduce((promiseChain, test) => {
...
let ioTestingForNonDynamicLanguage = function(data, testSet) {
    ...
    return compilationSrv.compile(data)
        .then(function(compResult) {
            if (!compResult.compilationError) {
                return ioTesting(data, testSet, compResult.id)
                    .then(function(testResult) {
                        return {
...
let testProjectForSubmission = function (data) {
    ...
    if (!testSet) Promise.reject("Error reading the tests");
...
exports.testProjectForSubmission = testProjectForSubmission;
