...
let ioTestSingle = function(data) {
    ...
    // array to string for ioMatching
    data.testData.input = arrayToStringWithLineBreaks(input);
...
let javaCompilationOutputGetNumErrors = function (output) {
    ...
    return (result && typeof result[0] !== 'undefined') ? result[0] : false;
};
...
let compilationOutputSeparation = function (output) {
    ...
    // esacape output from any line breaks to make regex easier
    output = escapeFromLineBreaks(output);
