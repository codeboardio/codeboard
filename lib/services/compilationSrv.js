 ...
 * todo same function in ioTestingSrv
 * @param output
 * @param regex
 ...
 */
let simpleCompilationErrorMessage = function(data) {

    let compileTests = [];

    let outputArray = (data.language === 'Java') ? compilationOutputSeparation(data.output) : [data.output];
    ...
    if(fs.existsSync(filePath)) {
        ...
        try {
            ...
            Promise.reject("Compile tests not found");
        }
    }
 ...
 *
 * todo aufpassen wegen data.stream = true?
 *
 * @param aCompilationError
 * @param projectId
 * @param userId
 ...
 * @param filesInDefaultFormat
 */
let onCompilationError = function(aCompilationError, projectId, userId, courseId, filesInDefaultFormat) {

    return createCompilationError(projectId, userId, courseId, filesInDefaultFormat, aCompilationError.output)
        .then(compilationError => {
            ...
            if(compilationError) {
                ...
                if(randomInt === 0) {
                    compMessagePromise = newCompilationErrorMessage(compilationError, compilationOutputSeparation(aCompilationError.output));
                } else {
                    compMessagePromise = simpleCompilationErrorMessage();
                }

                // resolve promise and return the compilation error6
                return compMessagePromise
                    .then(function(compilationErrorResponse) {
                        ...
                        .catch((err) => {
                            console.log(err);
                            return simpleCompilationErrorMessage()
                                .then(function(compilationErrorResponse) {
                                    compilationErrorAddResponse(compilationError, compilationErrorResponse);
                                    aCompilationError.compErrorHelpMessage =  compilationErrorResponse;
                                    aCompilationError.compilationErrorId =  compilationError.id;
    ...
    return mantraSrv.compile(data)
        .then(function(compResult) {
            ...
            if (compResult.compilationError && data.stream === false) {
                return onCompilationError(compResult, data.projectId, data.userId, data.courseId, data.filesInDefaultFormat)
                    .catch(function(err) {
